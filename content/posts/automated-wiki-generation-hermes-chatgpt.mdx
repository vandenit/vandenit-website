---
title: "Five Book Wikis in 48 Hours: An Automated Hermes + ChatGPT Pipeline With No API Keys"
date: 2026-09-08
...
Here's exactly how it works, what went wrong, and why I think the "no API keys" part is not just a cost trick — it's the reason the images came out consistent.
...
**1. I already pay for the capacity.** ChatGPT Plus includes scheduled tasks and image generation. Hermes runs on GLM through Ollama Cloud, which is cheap enough for an hourly cron with heavy file work. There is no per-image billing here — no OpenAI key, no FAL key. If you already have a Plus subscription, this is the difference between a new budget line and no budget line.

**2. Stateless API calls produce inconsistent characters.** This is the structural argument. A direct image API call receives exactly one thing: your prompt. It has no memory of what Arthur Dent looked like in the last thirty images. So every scene with Arthur is a dice roll — different face, different hair, different age. The usual fixes are negative-prompt gymnastics or building your own reference-image plumbing around the API.

The scheduler-driven approach solves it with context instead of plumbing. Before ChatGPT generates a scene image, it reads `projects.json` (project state), the project skill file (rules, naming, what exists), the queue item (exact instructions), and — critically — the canonical portraits of the characters in that scene, which it downloads from Drive and uses as identity references. Arthur Dent's portrait is generated once and becomes canon. Every scene image afterward is generated *with that portrait as a reference*. The same face shows up across the whole book, in dozens of images.

**3. Context beats prompt engineering.** Character consistency comes from context, not from the model. The GPT scheduler, fed the full project state through Drive, reliably produced more consistent images than I got from direct API calls with hand-tuned prompts and no context. That was the surprise: the "cheaper" option was also the better one.
...
- **0 API keys** created for image generation
