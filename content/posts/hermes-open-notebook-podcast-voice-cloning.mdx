...
date: 2026-07-15
excerpt: >
  Self-host your own NotebookLM with Open Notebook, Hermes Agent, and XTTS voice cloning. Generate podcasts with any voice — entirely from your phone.
...
2. **"Configure GLM 5 everywhere"** — Hermes read its own config for the Ollama Cloud API key, then made a dozen REST API calls to register credentials, models, and defaults in Open Notebook.
...
> **Note:** Separate tutorials for Hermes Web UI setup and Google Workspace integration are coming. This post focuses on the Open Notebook + voice cloning pipeline itself.
...
| LLM | GLM 5 on Ollama Cloud | OpenAI-compatible endpoint |
...
Then register the models — GLM 5 for language tasks and nomic-embed-text for embeddings:
...
VOICE_REFS = {
    "stephen_fry":   "~/voice-refs/stephen_fry.wav",
    "jon_lindstrom": "~/voice-refs/jon_lindstrom.wav",
    "lynn_chen":     "~/voice-refs/lynn_chen.wav",
    "scott_brick":   "~/voice-refs/scott_brick.wav",
    # Default fallback maps to Stephen Fry
    "alloy":         "~/voice-refs/stephen_fry.wav",
    "echo":          "~/voice-refs/jon_lindstrom.wav",
}
...
Then we create an episode profile that tells GLM 5 to write a natural conversation:
